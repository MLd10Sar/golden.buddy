...
const APP_TOKEN = 'gb_v52_ultra_fast'; 
...
export function useGoldenBuddyStore() {
  ...
  const syncRemoteData = useCallback(async () => {
    ...
    try {
      ...
      // SYNC OUTGOING INVITE STATUS (Check if buddy responded)
      ...
      for (const out of outgoingPending) {
        ...
        const cleaned = statusRaw.trim().replace(/^"(.*)"$/, '$1').replace(/\\"/g, '"');
        
        if (cleaned && cleaned !== "null") {
          try {
            const data = JSON.parse(cleaned); // Attempt to parse JSON payload
            if (data.status === 'ACCEPTED' || data.status === 'DECLINED') {
              setState(prev => ({
                ...prev,
                invites: prev.invites.map(i => i.id === out.id ? { 
                  ...i, 
                  status: data.status, 
                  coordinationNote: data.note, 
                  aiSuggestedSpot: data.spot ? JSON.stringify(data.spot) : undefined,
            ...
            if (cleaned === 'ACCEPTED' || cleaned === 'DECLINED') {
              setState(prev => ({
                ...prev,
                invites: prev.invites.map(i => i.id === out.id ? { ...i, status: cleaned as InviteStatus, respondedAt: Date.now() } : i)
              }));
            }
          }
        }
  ...
  useEffect(() => {
    if (!state.currentSession) return;
    broadcastPresence();
    const pInt = setInterval(broadcastPresence, 3000);
  ...
  const respondToInvite = async (inviteId: string, action: 'ACCEPTED' | 'DECLINED', note?: string, aiSpot?: any) => {
    if (!state.currentSession) return;

    // Update Local State
    setState(prev => ({
      ...prev,
      invites: prev.invites.map(inv => inv.id === inviteId ? { 
        ...inv, 
        status: action, 
        coordinationNote: note || inv.coordinationNote,
        aiSuggestedSpot: aiSpot ? JSON.stringify(aiSpot) : inv.aiSuggestedSpot,
        respondedAt: Date.now() 
      } : inv)
    }));

    try {
      // Prepare payload for the sender to see
      const payload = {
        status: action,
        note: note || "",
        spot: aiSpot || null
      ...
      await fetch(`${RELAY_BASE}/UpdateValue/${APP_TOKEN}/v_${inviteId}/${encodeURIComponent(JSON.stringify(payload))}`, { method: 'POST' });
  ...
  const updateAccessibility = (settings: Partial<AccessibilitySettings>) => {
    setState(prev => prev.currentSession ? { ...prev, currentSession: { ...prev.currentSession, accessibility: { ...prev.currentSession.accessibility, ...settings } } } : prev);
  };

  const updateInviteDuration = (durationMs: number) => {
    setState(prev => prev.currentSession ? { ...prev, currentSession: { ...prev.currentSession, inviteDuration: durationMs } } : prev);
  };

  const updateInviteNote = (inviteId: string, note: string) => {
    setState(prev => ({ ...prev, invites: prev.invites.map(inv => inv.id === inviteId ? { ...inv, coordinationNote: note } : inv) }));
  };
  ...
  return { 
    state, remotePeers, setState, setView: (view: View) => setState(prev => ({...prev, currentView: view})), 
    createSession, sendInvite, respondToInvite, updateInviteNote, resetApp, updateAccessibility, updateInviteDuration
  };
}
